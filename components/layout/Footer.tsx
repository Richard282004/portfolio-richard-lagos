    ...
    <footer className="border-t border-border pb-[calc(env(safe-area-inset-bottom,0px)+2rem)] pt-12">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-6 text-center">
        ...
        <div className="flex items-center gap-5">
          ...
          <a href={`mailto:${profile.socials.email}`} aria-label="Correo" className="text-fg-muted transition-all duration-300 hover:scale-110 hover:text-fg">
        ...
        <a
          href="#hero"
          className={`group/top inline-flex items-center gap-1.5 text-sm text-fg-muted transition-all duration-300 hover:text-fg ${
            showScrollTop ? "opacity-100" : "opacity-60"
          }`}
        >
          Volver arriba
          <ArrowUp className="size-3.5 transition-transform duration-300 group-hover/top:-translate-y-1" aria-hidden="true" />
        </a>
      </div>
    </footer>
  );
